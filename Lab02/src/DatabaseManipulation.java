...
public class DatabaseManipulation implements DataManipulation {
    ...
    public String findMovieById(int id) {
        getConnection();
        StringBuilder sb = new StringBuilder();
        String sql = "select m.title,c.country_name country,m.year_released ,m.runtime " +
                "from movies m " +
                "join countries c on m.country=c.country_code " +
                "where m.movieid = ?;";
        try {
            ...
            resultSet = preparedStatement.executeQuery();
            if (resultSet.next()) {
                sb.append("runtime:" + resultSet.getString("runtime") + System.lineSeparator());
                sb.append("country:" + resultSet.getString("country") + System.lineSeparator());
                sb.append("year released:" + resultSet.getString("year_released") + System.lineSeparator());
                sb.append("title:" + resultSet.getString("title") + System.lineSeparator());
            }
        } catch (SQLException e) {
            e.printStackTrace();
        } finally {
